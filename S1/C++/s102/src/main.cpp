...
int main(int, char**) // Version special du main, ne pas modifier
{
    ...
    // Question 1.1
    Personnage personnage(image_personnage, 6, 4, TAILLE_CASE, 2*TAILLE_CASE);
    ...
    while (!quitter)
    {
        ...
        if (moteur.animationsAmettreAjour())
        {
            ...
            //Question 2.3
            niveau.testerSpecial(ennemi1.getX(),ennemi1.getY());
            niveau.testerSpecial(ennemi2.getX(),ennemi2.getY());
        }
        //Question 2.5
        if (!niveau.ennemisActifs())
        {
